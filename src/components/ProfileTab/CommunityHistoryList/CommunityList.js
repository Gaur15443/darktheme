...
const CommunityList = () => {
  ...
  const handleCommunitySearch = text => {
    ...
    const results = Communities.filter(item =>
  ...
  useEffect(() => {
    if (Communities) {
      const results = Communities.filter(item => item.name.toLowerCase());
      setSearchCommunityResults(results);
    }
  }, []);

  const handleSelectCommunityItem = item => {
    route.params.onSelectCommunity(item); // Pass the selected community back
    navigation.goBack(); // Go back to the previous screen
  };

  const handleAddCustomCommunity = () => {
    const customItem = {id: Date.now(), name: communitySearchQuery, imgurl: ''}; // Add custom item
    route.params.onSelectCommunity(customItem);
    navigation.goBack();
  };
