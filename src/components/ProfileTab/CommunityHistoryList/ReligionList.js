...
const ReligionList = () => {
  ...
  const handleCommunitySearch = text => {
    ...
    const results = Religions.filter(item =>
  ...
  useEffect(() => {
    const results = Religions.filter(item => item.name.toLowerCase());
    setSearchCommunityResults(results);
  }, []);

  const handleSelectCommunityItem = item => {
    route.params.onSelectReligion(item); // Pass the selected community back
    navigation.goBack(); // Go back to the previous screen
  };

  const handleAddCustomCommunity = () => {
    const customItem = {id: Date.now(), name: communitySearchQuery, imgurl: ''}; // Add custom item
    route.params.onSelectReligion(customItem);
    navigation.goBack();
  };
